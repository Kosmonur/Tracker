...
//  Tracker
//
//  Created by Александр Пичугин on 23.09.2023.
//
...
final class EditTrackerViewController: UIViewController {
    ...
    init(_ trackerType: TrackerType, categoryName: String, tracker: Tracker) {
    ...
    private lazy var completedDays: UILabel = {
        let completedDays = UILabel()
        completedDays.font = Font.bold32
        completedDays.textColor = Color.ypBlack
        completedDays.textAlignment = .center
        completedDays.translatesAutoresizingMaskIntoConstraints = false
    ...
    private lazy var createButton: UIButton = {
        ...
        createButton.setTitle(Constant.save, for: .normal)
    ...
    private func setupContent() {
        ...
        scrollView.addSubview(completedDays)
        ...
        trackerNameField.text = trackerName
        
        let indexSelectedEmoji = Int(Constant.emojis.firstIndex(where: { $0 == tracker.emoji }) ?? 0)
        
        let uiColorMarshalling = UIColorMarshalling()
        let indexSelectedColor = Int(Color.colorsArray.firstIndex(where: { uiColorMarshalling.hexString(from: $0) == uiColorMarshalling.hexString(from: tracker.color) }) ?? 0)
        indexOfSelectedEmoji = [0, indexSelectedEmoji]
        indexOfSelectedColor = [1, indexSelectedColor]
        completedDays.text = "\(completedTrackerWithId) \(TextHelper.correctDaysWord(for: completedTrackerWithId))"
    }
    
    private func setupTypeTracker() {
        switch trackerType {
        case .habit:
            title = Constant.editHabbit
            tableView.heightAnchor.constraint(equalToConstant: 150).isActive = true
        case .event:
            title = Constant.editEvent
    ...
    private func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            completedDays.topAnchor.constraint(equalTo: scrollView.topAnchor, constant: 24),
            completedDays.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor, constant: 16),
            completedDays.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -16),
            
            trackerNameField.heightAnchor.constraint(equalToConstant: 75),
            trackerNameField.topAnchor.constraint(equalTo: completedDays.bottomAnchor, constant: 40),
...
extension EditTrackerViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if indexPath == indexOfSelectedEmoji {
                cell?.isSelected(true)
            }
        } else {
            cell?.cellLabel.backgroundColor = Color.colorsArray[indexPath.row]
            if indexPath == indexOfSelectedColor {
                cell?.isSelected(true)
            }
        }
